 ...
 * annotation to each getter of the class. If we do not do with, the field will
 * be passed into invoke() method.
 * <p/>
 * <strong>For developer</strong>:
 * <ul>
 * <li>
 * In default, the class field's name is equal to the data field's name. If you
 * want to map the class field into another data field, use the {@code MapField}
 * parameter please.
 * <p/>
 * For example:
 * <p/>
 ...
 * </code></li>
 * <li>
 * If the field is other class's key field, please use the {@code MapObject}
 * parameter.
 * <p/>
 * For example:
 * <p/>
 ...
 * otherwise rewrite the invoke method to deal with it.
 * <p/>
 * For example:
 * <p/>
 * <code>
...
public abstract class BaseCondition implements ApplicationContextAware {
	 ...
	 * <strong>USAGE</strong>:
	 * <p/>
	 * We can iterator all the fields which will be considered, and handle the
	 * condition object according to the fields' values.
	 * <p/>
	 * <strong>WARN</strong>:
	 * <p/>
	 ...
	 * 
	 * @param condition
	 *            conditions that to be considered
	 * @see Condition
	 * @see JoinedProperty
	 */
	 ...
	 *            whether columns' name begin uppercase letter first
	 * @return condition object we need
	 */
	...
	Condition getCondition(boolean upperCaseFirst) {
		...
		for (Method method : clazz.getMethods()) {
			if (method.getName().startsWith("get")) {
				try {
					String fieldName = StringUtil
							.getFieldNameFromGetterOrSetter(method.getName());
					...
					mapField = StringUtil.isNullOrWhiteSpace(mapField) ? fieldName
							: mapField;
					mapField = upperCaseFirst ? mapField.substring(0, 1)
							.toUpperCase() + mapField.substring(1) : mapField;
					...
					Criterion c = (Criterion) restrictionsClass.getMethod(
							exp.Type().name(), String.class, Object.class)
							.invoke(null, mapField, value);
					if (isJoinedProperty) {
						condition.addJoinedProperty(mapField,
								new JoinedProperty(c, keyValue, exp.Type()));
					} else {
