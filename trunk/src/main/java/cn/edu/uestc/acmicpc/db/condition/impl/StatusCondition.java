 ...
 */
public class StatusCondition extends BaseCondition implements
		UserConditionAware {
	/**
	 * Start status id.
	 */
	...
	/**
	 * <strong>WARN</strong>: if we set {@code contestId} with {@code -1}, that
	 * means we will query all records with contestId is {@code null}.
	 * 
	 * @param condition
		...
		if (result != null && !result.isEmpty() || resultId != null) {
			if (result != null && !result.isEmpty()) {
				...
				for (Global.OnlineJudgeReturnType onlineJudgeReturnType : result)
					junction.add(Restrictions.eq("result",
							onlineJudgeReturnType.ordinal()));
