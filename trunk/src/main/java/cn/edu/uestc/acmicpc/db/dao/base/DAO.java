...
public abstract class DAO<Entity extends Serializable, PK extends Serializable>
	 ...
	 * 
	 * @param criteria
	 *            Criteria object to update
	 * @param condition
	 *            conditions for criteria query
	 */
	private void updateCriteria(Criteria criteria, Condition condition) {
		if (condition.orders != null) {
			for (Condition.Order order : condition.orders) {
				criteria.addOrder(order.asc ? Order.asc(order.field) : Order
						.desc(order.field));
			}
		}
		criteria.setFirstResult(condition.getCurrentPage() == null ? 0
				: (int) ((condition.getCurrentPage() - 1) * condition
						.getCountPerPage()));
		if (condition.getCountPerPage() != null)
			criteria.setMaxResults((int) condition.getCountPerPage()
					.longValue());
		for (Criterion criterion : condition.getCriterionList()) {
			criteria.add(criterion);
		}
		for (JoinedProperty joinedProperty : condition.getJoinedProperties()
				.values()) {
	...
	public Entity getEntityByUniqueField(String fieldName, Object value,
		...
		try {
			for (Method method : methods) {
				...
				if (columnName != null && columnName.equals(fieldName)) {
					if (isUnique) {
						...
						throw new FieldNotUniqueException("Field '"
								+ propertyName + "' is not unique.");
					}
				}
			}
		} catch (HibernateException e) {
			...
			throw new AppException(
					"Invoke getEntityByUniqueField method error.");
		}
		return result;
	}
	...
	public String getSQLString(Condition condition) throws AppException {
		...
		for (String key : condition.getJoinedProperties().keySet()) {
			JoinedProperty joinedProperty = condition.getJoinedProperties()
					.get(key);
			String field;
	...
	public void updateEntitiesByCondition(Map<String, Object> properties,
		...
		stringBuilder.append("update ")
				.append(getReferenceClass().getSimpleName()).append(" set");
		Boolean first = true;
