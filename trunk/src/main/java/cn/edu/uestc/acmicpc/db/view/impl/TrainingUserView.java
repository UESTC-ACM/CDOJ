...
public class TrainingUserView extends View<TrainingUser> {
	...
	public TrainingUserView(TrainingUser trainingUser) {
		...
		setTypeName(Global.TrainingUserType.values()[trainingUser.getType()]
				.getDescription());
		...
		if (trainingStatusList.size() == 0)
			...
			setLastContestId(trainingStatusList
					.get(trainingStatusList.size() - 1)
					.getTrainingContestByTrainingContestId()
					.getTrainingContestId());
		}
	}
}
