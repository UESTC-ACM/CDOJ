...
public class ArticlePictureAdminAction extends FileUploadAction implements ArticleDAOAware {
  ...
  public String uploadPicture() {
    try {
      ...
      File newFile =
          new File(settings.SETTING_ARTICLE_PICTURE_FOLDER_ABSOLUTE + targetArticleId + "/"
              + newName);
      ...
      json.put("uploadedFileUrl", settings.SETTING_ARTICLE_PICTURE_FOLDER + targetArticleId + "/"
          + newName);
    } catch (AppException e) {
      json.put("error", e.getMessage());
    } catch (Exception e) {
