...
public class ProblemAdminAction extends BaseAction implements
		ProblemConditionAware, ProblemDAOAware, ProblemDTOAware {
		...
		try {
			if (targetProblemId == null) {
				...
				if (count == 0) {
					...
					List<Problem> result = (List<Problem>) problemDAO
				...
				return redirect(getActionURL("/admin", "problem/editor/"
						+ targetProblemId));
			} else {
									...
									String setterName = StringUtil
											.getGetterOrSetter(
													...
													getter.getName().substring(
															3));
									Method setter = problem.getClass()
											.getMethod(setterName,
													getter.getReturnType());
									setter.invoke(problem, ReflectionUtil
											.valueOf(value,
													getter.getReturnType()));
								}
							}
