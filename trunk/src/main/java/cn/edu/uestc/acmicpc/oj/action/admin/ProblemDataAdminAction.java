...
public class ProblemDataAdminAction extends FileUploadAction implements
		ProblemDAOAware, ProblemDataDTOAware {
	/**
	 * 
	 */
	...
	private String getDataZipFileName() {
		return settings.SETTING_UPLOAD_FOLDER + "/problem_" + targetProblemId
				+ ".zip";
	}
	...
	public String toProblemDataEditor() {
		try {
			problemDataView = new ProblemDataView(
					problemDAO.get(targetProblemId));
	 ...
	 * <strong>INFO</strong>:
	 * <p/>
	 * All files in data folder should named from 1 to number of cases, input
	 * files end with ".in" and output files end with ".out".
	 * 
	...
	/**
	 * To update problem test information and compile spj checker in the data
	 * folder.
	 * <p/>
	...
	@Validations(intRangeFields = {
			@IntRangeFieldValidator(fieldName = "problemDataDTO.timeLimit", min = "0", max = "60000", key = "error.timeLimit.validation"),
			@IntRangeFieldValidator(fieldName = "problemDataDTO.javaTimeLimit", min = "0", max = "60000", key = "error.timeLimit.validation"),
			@IntRangeFieldValidator(fieldName = "problemDataDTO.memoryLimit", min = "0", max = "262144", key = "error.memoryLimit.validation"),
			@IntRangeFieldValidator(fieldName = "problemDataDTO.javaMemoryLimit", min = "0", max = "262144", key = "error.memoryLimit.validation"),
			@IntRangeFieldValidator(fieldName = "problemDataDTO.outputLimit", min = "0", max = "262144", key = "error.outputLimit.validation") })
		...
		try {
			...
			if (dataCount != 0) {
				...
				for (String file : fileMap.keySet()) {
					...
					File toFile = new File(dataPath + '/' + fileMap.get(file)
							+ ".in");
					if (!fromFile.renameTo(toFile))
						throw new AppException("Cannot rename file: " + file
								+ ".in");
					fromFile = new File(dataPath + '/' + file + ".out");
					toFile = new File(dataPath + '/' + fileMap.get(file)
							+ ".out");
					if (!fromFile.renameTo(toFile))
						throw new AppException("Cannot rename file: " + file
								+ ".out");
