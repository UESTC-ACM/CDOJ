...
public class ProblemPictureAdminAction extends FileUploadAction implements ProblemDAOAware {
    ...
    public String toPictureList() {
        try {
            ...
            File dir = new File(settings.SETTING_PROBLEM_PICTURE_FOLDER + targetProblemId + "/");
            if (!dir.exists())
                if (!dir.mkdirs())
            ...
            for (File file : files) {
                ...
                if (iter.hasNext())
                    pictures.add("/images/problem/" + targetProblemId + "/" + fileName);
            }
    ...
    public String uploadPicture() {
        try {
            ...
            File newFile = new File(settings.SETTING_PROBLEM_PICTURE_FOLDER + targetProblemId + "/" + newName);
            ...
            json.put("uploadedFileUrl", "/images/problem/" + targetProblemId + "/" + newName);
        } catch (AppException e) {
            json.put("error", e.getMessage());
        } catch (Exception e) {
            e.printStackTrace();
