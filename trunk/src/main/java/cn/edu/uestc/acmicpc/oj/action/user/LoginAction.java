...
public class LoginAction extends BaseAction implements StatusConditionAware,
	...
	/**
	 * toLogin with {@code userName} and {@code password}. <strong>JSON
	 * output</strong>:
	 * <ul>
	 * <li>
	...
	@Validations(requiredStrings = {
			...
			@RequiredStringValidator(fieldName = "password", key = "error.password.validation") }, stringLengthFields = { @StringLengthFieldValidator(fieldName = "password", key = "error.password.validation", minLength = "6", maxLength = "20", trim = false) }, customValidators = { @CustomValidator(type = "regex", fieldName = "userName", key = "error.userName.validation", parameters = {
	...
	public String toLogin() {
		try {
			User user = userDAO.getEntityByUniqueField("userName",
					getUserName());
			if (user == null
					|| !StringUtil.encodeSHA1(getPassword()).equals(
							user.getPassword())) {
