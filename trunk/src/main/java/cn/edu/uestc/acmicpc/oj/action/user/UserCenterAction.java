...
public class UserCenterAction extends BaseAction implements StatusDAOAware,
		StatusConditionAware, ProblemDAOAware, UserDTOAware,
		DepartmentDAOAware, ProblemConditionAware {
	 ...
	 * <ul>
	 * <li>
	 * For success: {"result":"ok", "problemStatus":<strong>ProblemStatus
	 * object</strong>, "problemCount":<strong>Tot problems</strong>}</li>
	 * <li>
	...
	public String toUserProblemState() {
		try {
			...
			List<Integer> results = (List<Integer>) problemDAO
					.findAll(condition);
	...
	@Validations(requiredStrings = {
			...
			@RequiredStringValidator(fieldName = "userDTO.studentId", key = "error.studentId.validation"),
			@RequiredStringValidator(fieldName = "userDTO.nickName", key = "error.nickName.validation"),
			@RequiredStringValidator(fieldName = "userDTO.oldPassword", key = "error.password.validation") }, stringLengthFields = {
			...
			@StringLengthFieldValidator(fieldName = "userDTO.oldPassword", key = "error.password.validation", minLength = "6", maxLength = "20", trim = false) }, customValidators = { @CustomValidator(type = "regex", fieldName = "userDTO.nickName", key = "error.nickName.validation", parameters = {
			@ValidationParameter(name = "expression", value = "\\b^[^\\s]{2,20}$\\b"),
			@ValidationParameter(name = "trim", value = "false") }) }, fieldExpressions = { @FieldExpressionValidator(fieldName = "userDTO.departmentId", expression = "userDTO.departmentId in global.departmentList.{departmentId}", key = "error.department.validation") })
	@LoginPermit(NeedLogin = true)
	public String toEdit() {
		try {
			if (userDTO.getUserId() == null
					|| !userDTO.getUserId().equals(currentUser.getUserId()))
				throw new AppException("You can only change your profile!");
			User user = userDAO.get(userDTO.getUserId());
			if (user == null)
				throw new AppException("No such user!");
			if (!StringUtil.encodeSHA1(userDTO.getOldPassword()).equals(
					user.getPassword())) {
				addFieldError("userDTO.oldPassword",
						"Your passowrd is wrong, please try again.");
				return INPUT;
			}
			if (userDTO.getPassword() != null) {
				if (userDTO.getPassword().length() < 6
						|| userDTO.getPassword().length() > 20) {
					addFieldError("userDTO.password",
							"Please enter 6-20 characters.");
					return INPUT;
				}
				if (!userDTO.getPassword().equals(userDTO.getPasswordRepeat())) {
					addFieldError("userDTO.passwordRepeat",
