...
public class XmlNode implements Iterable<XmlNode> {
	...
	public void addAttribute(String key, String value) throws AppException {
		if (getAttributes() == null)
			...
			throw new AppException(
					"There is same attribute in the attribute list.");
		else
	...
	public void removeAttribute(String key) throws AppException {
		if (getAttributes() == null)
			...
			throw new AppException(
					"There is such attribute in the attribute list.");
		else
			getAttributes().remove(key);
	}

	/**
	 * Get specified attribute innerText from the attribute list, if the
	 * attribute list is {@code null} or there is no such attribute in the
	 * attribute list, return empty string, otherwise return the innerText of
	 * this attribute.
	 * 
	 * @param key
	 *            the attribute name
	...
	/**
	 * Add a new child node into the child nodes list, if the child node or
	 * child nodes list is {@code null}, or this is same node in the child node
	 * list, throw {@link AppException}.
	 * 
	 * @param child
	 *            new child node
	 * @throws AppException
	...
	public void addChild(XmlNode child) throws AppException {
		...
		for (XmlNode node : getChildList())
			if (node == child)
				throw new AppException(
						"This node is contained in the child node list.");
