...
public class RatingUtil implements TrainingUserDAOAware, TrainingStatusDAOAware {
	public void updateRating(TrainingContest trainingContest)
			throws AppException {
		if (trainingContest.getType() == Global.TrainingContestType.ADJUST
				.ordinal()) {
			for (TrainingStatus trainingStatus : trainingContest
					.getTrainingStatusesByTrainingContestId()) {
				TrainingUser trainingUser = trainingStatus
						.getTrainingUserByTrainingUserId();

				trainingUser.setRating(trainingUser.getRating()
						- trainingStatus.getPenalty());
				trainingUser.setRatingVary(-1.0 * trainingStatus.getPenalty());
				trainingUser
						.setCompetitions(trainingUser.getCompetitions() + 1);
			...
			if (trainingContest.getType() == Global.TrainingContestType.TEAM
					.ordinal()) {
				updateNewTrainingUser(newTrainingUsers, newTrainingUserRank,
						oldTrainingUsers, oldTrainingUserRank, 0.5);
				updateOldTrainingUser(oldTrainingUsers, oldTrainingUserRank,
						0.5);
			} else {
				updateNewTrainingUser(newTrainingUsers, newTrainingUserRank,
						oldTrainingUsers, oldTrainingUserRank, 1.0);
				updateOldTrainingUser(oldTrainingUsers, oldTrainingUserRank,
						1.0);
			}
			for (TrainingStatus trainingStatus : trainingContest
				...
				trainingStatus.setVolatilityVary(trainingUser
						.getVolatilityVary());
	...
	private void updateNewTrainingUser(List<TrainingUser> trainingUsers,
			List<Integer> rank, List<TrainingUser> oldUsers,
			List<Integer> oldRank, Double contestWeight) {
		if (trainingUsers.size() <= 1) {
			...
			for (int i = 0; i < trainingUsers.size(); i++) {
				...
				for (TrainingUser trainingUser : trainingUsers) {
					...
					eRank[i] += 0.5 * erf((trainingUser.getRating() - trainingUserA
							.getRating())
							/ Math.sqrt(2.0 * (square(trainingUserA
									.getVolatility()) + square(trainingUser
									.getVolatility())))) + 0.5;
				}
				for (TrainingUser oldUser : oldUsers) {
					TrainingUser trainingUserA = trainingUsers.get(i);
					eRank[i] += 0.5 * erf((oldUser.getRating() - trainingUserA
							.getRating())
							/ Math.sqrt(2.0 * (square(trainingUserA
									.getVolatility()) + square(oldUser
									.getVolatility())))) + 0.5;
				}
			}
			...
			for (int i = 0; i < trainingUsers.size(); i++) {
				aPerf[i] = -inv_norm((aRank[i] - 0.5)
						/ (trainingUsers.size() + oldUsers.size()));
				ePerf[i] = -inv_norm((eRank[i] - 0.5)
						/ (trainingUsers.size() + oldUsers.size()));
			}
			for (int i = 0; i < trainingUsers.size(); i++) {
				TrainingUser trainingUser = trainingUsers.get(i);
				perfAs[i] = trainingUser.getRating() + cf
						* (aPerf[i] - ePerf[i]);
				Double weight = 1.0 / (1 - (0.42 / (trainingUser
						.getCompetitions() + 1) + 0.18)) - 1.0;
				if (trainingUser.getRating() >= 2000
						&& trainingUser.getRating() <= 2500)
					weight *= 0.9;
				else if (trainingUser.getRating() > 2500)
					weight *= 0.8;
				weight *= contestWeight;
				Double cap = 150.0 + 1500.0 / (trainingUser.getCompetitions() + 2.0);
				Double newRating = (trainingUser.getRating() + weight
						* perfAs[i])
						/ (1.0 + weight);
				newRating = Math.min(newRating, trainingUser.getRating() + cap);
				newRating = Math.max(newRating, trainingUser.getRating() - cap);
				Double newVolatility = Math.sqrt(square(newRating
						- trainingUser.getRating())
						/ weight
				...
				trainingUser
						.setRatingVary(newRating - trainingUser.getRating());
				trainingUser.setVolatilityVary(newVolatility
						- trainingUser.getVolatility());
	...
	private void updateOldTrainingUser(List<TrainingUser> trainingUsers,
			List<Integer> rank, Double contestWeight) {
		if (trainingUsers.size() <= 1) {
			for (TrainingUser trainingUser : trainingUsers) {
				trainingUser.setRatingVary(0.0);
				trainingUser.setVolatilityVary(0.0);
			...
			Double cf = Math.sqrt(a / trainingUsers.size() + b
					/ (trainingUsers.size() - 1));
			Double[] aRank = new Double[trainingUsers.size()];
			for (int i = 0; i < trainingUsers.size(); i++) {
				int lose = 0, tie = 0;
				for (int j = 0; j < trainingUsers.size(); j++)
