...
public class TrainingRankListParser implements TrainingUserDAOAware {
	...
	public TrainingContestRankList parse(TrainingContest trainingContest)
			throws ParserException, BiffException, AppException,
			FieldNotUniqueException, IOException {
	...
	public TrainingContestRankList parse(File file, Boolean isPersonal,
			Integer type) throws IOException, BiffException,
			FieldNotUniqueException, ParserException, AppException {
	 ...
	 * @throws AppException
	 */
	public TrainingContestRankList parse(List<String[]> excelValueList,
			Boolean isPersonal, Integer type) throws IOException,
			BiffException, ParserException, FieldNotUniqueException,
			AppException {
		if (excelValueList == null || excelValueList.size() == 0)
			throw new ParserException(
					"Error while parse xls document, Please check it!");
		...
		for (int i = 0; i < header.length; i++) {
			...
			if (header[i].compareToIgnoreCase("rank") == 0) {
				if (headerMap.containsKey("rank"))
					throw new ParserException(
							"There are multiple columns reference to rank");
			...
			if (header[i].compareToIgnoreCase("name") == 0
				...
				if (headerMap.containsKey("name"))
					throw new ParserException(
							"There are multiple columns reference to name");
			...
			if (header[i].compareToIgnoreCase("solved") == 0
				...
				if (headerMap.containsKey("solved"))
					throw new ParserException(
							"There are multiple columns reference to solved");
			...
			if (header[i].compareToIgnoreCase("penalty") == 0) {
				if (headerMap.containsKey("penalty"))
					throw new ParserException(
							"There are multiple columns reference to penalty");
		...
		if (type != Global.TrainingContestType.NORMAL.ordinal()
			...
			if (!headerMap.containsKey("penalty"))
				throw new ParserException(
						"There are no columns reference to penalty");
			...
			return new TrainingContestRankList(excelRankList, isPersonal,
					trainingUserDAO, type);
		} else {
			...
			for (int i = 1; i < excelValueList.size(); i++) {
				...
				if (!problemIterator.equals(problemCount))
					throw new ParserException("Rank list format error, Row "
							+ i + " contains " + problemIterator
							+ " problems but there are " + problemCount
							+ " problem in total");
				excelRankList.add(newLine);
			}
			return new TrainingContestRankList(excelRankList, isPersonal,
	...
	public List<String[]> parseDatabase(TrainingContest trainingContest)
			throws ParserException {
		...
		for (TrainingStatus trainingStatus : trainingContest
			...
			if (trainingContest.getType() != Global.TrainingContestType.NORMAL
					.ordinal()
					&& trainingContest.getType() != Global.TrainingContestType.TEAM
							.ordinal()) {
				String[] result = new String[2];
				result[0] = trainingStatus.getTrainingUserByTrainingUserId()
						.getName();
				result[1] = trainingStatus.getPenalty().toString();
				valueList.add(result);
			} else {
				String[] summary = parseTrainingUserSummary(trainingStatus
						.getSummary());
				...
				if (summaryLength == -1)
					...
					throw new ParserException(
							"Summary in database length different error");
			}
		}
		if (trainingContest.getType() != Global.TrainingContestType.NORMAL
				.ordinal()
	...
	public String encodeTariningUserSummary(
		...
		for (TrainingProblemSummaryInfo trainingProblemSummaryInfo : trainingProblemSummaryInfos) {
			...
			if (!trainingProblemSummaryInfo.getSolved()) {
				if (trainingProblemSummaryInfo.getTried() > 0)
					stringBuilder.append(trainingProblemSummaryInfo.getTried())
							.append("/--");
			} else
				stringBuilder.append(trainingProblemSummaryInfo.getTried())
						.append("/")
