...
(function(){var a,b;a={OnlineJudgeReturnType:{OJ_WAIT:0,OJ_AC:1,OJ_PE:2,OJ_TLE:3,OJ_MLE:4,OJ_WA:5,OJ_OLE:6,OJ_CE:7,OJ_RE_SEGV:8,OJ_RE_FPE:9,OJ_RE_BUS:10,OJ_RE_ABRT:11,OJ_RE_UNKNOWN:12,OJ_RF:13,OJ_SE:14,OJ_RE_JAVA:15,OJ_JUDGING:16,OJ_RUNNING:17,OJ_REJUDGING:18},ContestType:{PUBLIC:0,PRIVATE:1,DIY:2,INVITED:3},ContestStatus:{PENDING:"Pending",RUNNING:"Running",ENDED:"Ended"},AuthenticationType:{NORMAL:"0",ADMIN:"1",CONSTANT:"2"},AuthorStatusType:{NONE:0,PASS:1,FAIL:2}};b=angular.module("cdoj",["ngSanitize","monospaced.elastic","ui.bootstrap"]);b.run(["$rootScope","$http",function(c,d){_.extend(c,a);return d.get("/globalData").then(function(e){var f;f=e.data;if(f.result==="error"){return alert(f.error_msg)}else{return _.extend(c,f)}})}]);b.controller("ActivateController",["$scope","$http","$element","$window",function(d,f,c,e){d.userName="";d.buttonText="Send Email";d.onSend=false;return d.sendSerialKey=function(){var g;if(d.userName.length>0){d.buttonText="Sending...";d.onSend=true;g=angular.copy(d.userName);return f.post("/user/sendSerialKey/"+g).then(function(h){var i;i=h.data;if(i.result==="success"){e.alert("We send you an Email with the url to reset your password right now, please check your mail box.");c.modal("hide")}else{if(i.result==="failed"){e.alert("Unknown error occurred.")}else{e.alert(i.error_msg)}}d.buttonText="Send Email";return d.onSend=false})}}}]);b.controller("ArticleController",["$scope","$rootScope","$http","$window",function(d,c,f,e){d.article={content:"",title:""};d.articleId=0;return d.$watch("articleId",function(){var g;g=angular.copy(d.articleId);return f.get("/article/data/ArticleDTO/"+g).then(function(h){var i;i=h.data;if(i.result==="success"){d.article=i.article;return c.title=d.article.title}else{return e.alert(i.error_msg)}})})}]);b.controller("ArticleEditorController",["$scope","$http","$window",function(c,e,d){c.article={content:"",title:""};c.action="new";c.fieldInfo=[];c.$watch("action",function(){var f;if(c.action!=="new"){f=angular.copy(c.action);return e.get("/article/data/ArticleEditorShowDTO/"+f).then(function(g){var h;h=g.data;if(h.result==="success"){return c.article=h.article}else{return d.alert(h.error_msg)}})}});return c.submit=function(){var f;f=angular.copy(c.article);f.action=angular.copy(c.action);return e.post("/article/edit",f).then(function(g){var h;h=g.data;if(h.result==="success"){return d.location.href="/article/show/"+h.articleId}else{if(h.result==="field_error"){return c.fieldInfo=h.field}else{return d.alert(h.error_msg)}}})}}]);b.controller("ContestController",["$scope","$rootScope","$http","$window","$modal","$interval",function(e,c,j,i,f,h){var g,d;e.contestId=0;e.contest={title:""};e.problemList=[];e.currentProblem={description:"",title:"",input:"",output:"",sampleInput:"",sampleOutput:"",hint:"",source:""};e.$watch("contestId",function(){var k;k=angular.copy(e.contestId);j.get("/contest/data/"+k).then(function(l){var m;m=l.data;if(m.result==="success"){e.contest=m.contest;e.problemList=m.problemList;c.title=m.contest.title;if(m.problemList.length>0){return e.currentProblem=m.problemList[0]}}else{return i.alert(m.error_msg)}});return d()});d=function(){var k;k=angular.copy(e.contestId);console.log(k);return j.get("/contest/rankList/"+k).then(function(l){var m;m=l.data;if(m.result==="success"){e.rankList=m.rankList.rankList;return _.each(e.problemList,function(o,n){o.tried=m.rankList.problemList[n].tried;return o.solved=m.rankList.problemList[n].solved})}})};g=h(d,10000);e.showProblemTab=function(){return e.$$childHead.tabs[1].select()};e.showStatusTab=function(){return e.$$childHead.tabs[3].select()};e.chooseProblem=function(k){e.showProblemTab();return e.currentProblem=_.findWhere(e.problemList,{order:k})};return e.openSubmitModal=function(){return f.open({templateUrl:"submitModal.html",controller:"SubmitModalController",resolve:{submitDTO:function(){return{codeContent:"",problemId:e.currentProblem.problemId,contestId:e.contest.contestId,languageId:2}},title:function(){return""+e.currentProblem.orderCharacter+" - "+e.currentProblem.title}}}).result.then(function(k){if(k==="success"){e.showStatusTab();return angular.element("#status-list").scope().refresh()}})}}]);b.directive("uiContestProblemHref",function(){return{restrict:"E",scope:{problemId:"=",problemList:"="},controller:["$scope",function(c){c.order=-1;c.orderCharacter="-";c.$watch("problemId + problemList",function(){var d;d=_.findWhere(c.problemList,{problemId:c.problemId});if(d!==void 0){c.orderCharacter=d.orderCharacter;return c.order=d.order}});return c.select=function(){if(c.order!==-1){return angular.element("#contest-show").scope().chooseProblem(c.order)}}}],template:'<a href="#" ng-bind="orderCharacter" ng-click="select()"></a>',replace:true}});b.controller("ContestEditorController",["$scope","$http","$window",function(c,e,d){c.contest={action:0,contestId:0,title:0,type:0,time:0,lengthHours:0,lengthDays:0,lengthMinutes:0,description:0,problemList:0};c.problemList=[];c.$watch("problemList",function(){return c.contest.problemList=_.map(c.problemList,function(f){return f.problemId}).join(",")},true);c.updateProblemTitle=function(f){if(f.problemId===void 0){return f.title="Invalid problem id!"}else{return e.get("/problem/query/"+f.problemId+"/title").then(function(g){var h;h=g.data;if(h.result==="success"){if(h.list.length===1){return f.title=h.list[0]}else{return f.title="No such problem!"}}else{return f.title=h.error_msg}})}};c.addProblem=function(){var h,f,g;g="";if(c.problemList.length>0){h=parseInt(c.problemList.last().problemId);if(!isNaN(h)){g=h+1}}f={problemId:g,title:""};c.updateProblemTitle(f);return c.problemList.add(f)};c.removeProblem=function(f){return c.problemList.splice(f,1)};return c.submit=function(){var f;f=angular.copy(c.contest);f.time=Date.create(f.time).getTime();console.log(c.contest);return e.post("/contest/edit",f).success((function(g){return function(h){if(h.result==="success"){return d.location.href="/contest/show/"+h.contestId}else{return d.alert(h.error_msg)}}})(this))}}]);b.controller("ListController",["$scope","$rootScope","$http",function(d,c,e){d.condition=0;d.list=[];d.pageInfo={countPerPage:20,currentPage:1,displayDistance:2,totalPages:1};d.requestUrl=0;d.reset=function(){_.each(d.condition,function(g,f){return d.condition[f]=void 0});return d.condition.currentPage=null};d.refresh=function(){var f;if(d.requestUrl!==0){f=angular.copy(d.condition);return e.post(d.requestUrl,f).then((function(g){return function(h){d.list=h.data.list;return d.pageInfo=h.data.pageInfo}})(this))}};c.$watch("currentUser",function(){return d.refresh()},true);return d.$watch("condition",function(){return d.refresh()},true)}]);b.controller("navBarController",["$scope","$window",function(c,d){return c.isActive=function(e){return e===d.location.pathname.split("/")[1]}}]);b.controller("PasswordResetController",["$scope","$http","$window",function(c,e,d){c.userActivateDTO={userName:"",serialKey:"",password:"",passwordRepeat:""};return c.submit=function(){var g,h,f;f=angular.copy(c.userActivateDTO);g=CryptoJS.SHA1(f.password).toString();f.password=g;h=CryptoJS.SHA1(f.passwordRepeat).toString();f.passwordRepeat=h;return e.post("/user/resetPassword",f).then(function(i){var j;j=i.data;if(j.result==="success"){d.alert("Success!");return d.location.href="/"}else{if(j.result==="field_error"){return c.fieldInfo=j.field}else{return d.alert(j.error_msg)}}})}}]);b.controller("ProblemController",["$scope","$rootScope","$http","$window",function(d,c,f,e){d.problem={description:"",title:"",isSpj:false,timeLimit:1000,javaTimeLimit:3000,memoryLimit:"--",javaMemoryLimit:"--",solved:0,tried:0,input:"",output:"",sampleInput:"",sampleOutput:"",hint:"",source:""};d.submitDTO={codeContent:"",languageId:1,contestId:0,problemId:0};d.problemId=0;d.contest=null;d.fieldInfo=[];d.$watch("problemId",function(){var g;g=angular.copy(d.problemId);return f.post("/problem/data/"+g).then(function(j){var m,l,k,h;m=j.data;if(m.result==="success"){d.problem=m.problem;k=d.problem.sampleInput;try{k=JSON.parse(k)}catch(n){}if(!(k instanceof Array)){k=[k]}h=d.problem.sampleOutput;try{h=JSON.parse(h)}catch(n){}if(!(h instanceof Array)){h=[h]}if(k.length!==h.length){alert("Sample input has not same number of cases with sample output!")}else{d.samples=(function(){var p,o,i;i=[];for(l=p=0,o=k.length-1;0<=o?p<=o:p>=o;l=0<=o?++p:--p){i.push({input:k[l].toString(),output:h[l].toString()})}return i})()}return c.title=d.problem.title}else{return alert(m.error_msg)}})});return d.submitCode=function(){var g;g=angular.copy(d.submitDTO);g.problemId=d.problemId;g.contestId=d.contestId;console.log(g);if(g.codeContent===void 0){return}if(c.hasLogin===false){return e.alert("Please login first!")}else{return f.post("/status/submit",g).then(function(h){var i;i=h.data;if(i.result==="success"){return e.location.href="/status/list"}else{if(i.result==="field_error"){return d.fieldInfo=i.field}else{return e.alert(i.error_msg)}}})}}}]);b.controller("ProblemEditorController",["$scope","$http","$window",function(c,e,d){c.action="new";c.problem={description:"",title:"",isSpj:false,timeLimit:1000,javaTimeLimit:3000,memoryLimit:65535,javaMemoryLimit:65535,outputLimit:65535,isVisible:false,input:"",output:"",sampleInput:"",sampleOutput:"",hint:"",source:""};c.fieldInfo=[];c.$watch("action",function(){var f;if(c.action!=="new"){f=angular.copy(c.action);return e.post("/problem/data/"+f).then(function(g){var h;h=g.data;if(h.result==="success"){return c.problem=h.problem}else{return d.alert(h.error_msg)}})}});return c.submit=function(){var f;f=angular.copy(c.problem);f.action=angular.copy(c.action);return e.post("/problem/edit",f).then(function(g){var h;h=g.data;if(h.result==="success"){return d.location.href="/problem/show/"+h.problemId}else{if(h.result==="field_error"){return c.fieldInfo=h.field}else{return d.alert(h.error_msg)}}})}}]);b.controller("RegisterController",["$scope","$rootScope","$http","$element","$window",function(e,c,g,d,f){e.userRegisterDTO={departmentId:1,email:"",motto:"",nickName:"",password:"",passwordRepeat:"",school:"",studentId:"",userName:""};e.fieldInfo=[];return e.register=function(){var i,j,h;h=angular.copy(e.userRegisterDTO);if(h.password===void 0){return}if(h.passwordRepeat===void 0){return}i=CryptoJS.SHA1(h.password).toString();h.password=i;j=CryptoJS.SHA1(h.passwordRepeat).toString();h.passwordRepeat=j;return g.post("/user/register",h).then(function(k){var l;l=k.data;if(l.result==="success"){c.hasLogin=true;c.currentUser={userName:l.userName,email:l.email,type:l.type};return d.modal("hide")}else{if(l.result==="field_error"){return e.fieldInfo=l.field}else{return f.alert(l.error_msg)}}})}}]);b.controller("SubmitModalController",["$scope","$rootScope","$window","$http","$modalInstance","submitDTO","title",function(d,c,h,i,f,e,g){d.submitDTO=e;d.title=g;d.submit=function(){e=angular.copy(d.submitDTO);if(e.codeContent===void 0){return}if(c.hasLogin===false){return h.alert("Please login first!")}else{return i.post("/status/submit",e).then(function(j){var k;k=j.data;if(k.result==="success"){return f.close("success")}else{if(k.result==="field_error"){return d.fieldInfo=k.field}else{return h.alert(k.error_msg)}}})}};return d.close=function(){return f.dismiss("close")}}]);b.controller("UserCenterController",["$scope","$rootScope","$http",function(d,c,e){d.targetUserName="";d.targetUser={email:""};return d.$watch("targetUserName",function(){var f;f=angular.copy(d.targetUserName);return e.get("/user/userCenterData/"+f).then(function(g){var h;h=g.data;if(h.result==="success"){d.targetUser=h.targetUser;return d.problemStatus=h.problemStatus}else{return $window.alert(h.error_msg)}})})}]);b.controller("UserController",["$scope","$rootScope","$http","$element","$compile","$window","UserProfile",function(e,c,i,d,f,h,g){c.hasLogin=false;c.currentUser={email:""};c.isAdmin=false;e.userLoginDTO={userName:"",password:""};e.fieldInfo=[];e.views={loginView:'<a href="#" class="dropdown-toggle" data-toggle="dropdown">\n  Sign in\n</a>\n<ul ui-dropdown-menu class="dropdown-menu cdoj-form-menu" style="width: 340px;">\n  <li>\n    <form>\n      <div class="input-group form-group input-group-sm">\n        <span class="input-group-addon">\n          <i class="fa fa-user" style="width: 14px;"></i>\n        </span>\n        <input type="text"\n               ng-model="userLoginDTO.userName"\n               maxlength="24"\n               id="userName"\n               class="form-control"\n               ng-required="true"\n               ng-pattern="/^[a-zA-Z0-9_]{4,24}$/"\n               placeholder="Username"/>\n      </div>\n      <div class="input-group form-group input-group-sm">\n        <span class="input-group-addon">\n          <i class="fa fa-key" style="width: 14px;"></i>\n        </span>\n        <input type="password"\n               ng-model="userLoginDTO.password"\n               id="password"\n               ng-required="true"\n               ng-minlength="6"\n               ng-maxlength="24"\n               class="form-control"\n               placeholder="Password"/>\n        <span class="input-group-btn">\n          <button type="submit"\n                  class="btn btn-default"\n                  ng-click="login()">\n            Login\n          </button>\n        </span>\n      </div>\n      <ui-validate-info value="fieldInfo" for="password"></ui-validate-info>\n    </form>\n  </li>\n  <li role="presentation" class="divider"></li>\n  <li>\n    <a href="#" data-toggle="modal"\n       data-target="#cdoj-register-modal">\n      <i class="fa fa-arrow-circle-right" style="padding-right: 6px;"></i>Register</a>\n    <a href="#" data-toggle="modal"\n       data-target="#cdoj-activate-modal">\n      <i class="fa fa-arrow-circle-right" style="padding-right: 6px;"></i>Forget password? </a>\n  </li>\n</ul>',userView:'<div id="cdoj-user">\n  <a href="#" class="dropdown-toggle" data-toggle="dropdown">\n    <img id="cdoj-user-avatar"\n         ui-avatar\n         email="currentUser.email"\n         src="/images/avatar/default.jpg"/>\n  </a>\n  <ul class="dropdown-menu"\n      role="menu"\n      aria-labelledby="user-menu">\n    <li role="presentation"\n        class="dropdown-header text-center">\n      <span id="currentUser"\n            ng-bind="currentUser.userName">\n        </span>\n    </li>\n    <li role="presentation">\n      <a href="/user/center/{{currentUser.userName}}">\n        <i class="fa fa-home"></i>User center\n      </a>\n    </li>\n    <li role="presentation">\n      <a href="#" data-toggle="modal"\n         data-target="#cdoj-profile-edit-modal"\n         ng-click="setupProfileEditor()">\n        <i class="fa fa-wrench"></i>Edit profile\n      </a>\n    </li>\n    <li role="presentation" class="divider"></li>\n    <li role="presentation">\n      <a href="#" id="cdoj-logout-button" ng-click="logout()">\n        <i class="fa fa-power-off"></i>Logout\n      </a>\n    </li>\n  </ul>\n</div>'};c.$watch("hasLogin",function(){var j;if(c.hasLogin){j=e.views.userView}else{j=e.views.loginView}d.html(j);f(d.contents())(e);if(c.hasLogin&&c.currentUser.type===1){return c.isAdmin=true}else{return c.isAdmin=false}});e.login=function(){var k,j;j=angular.copy(e.userLoginDTO);if(j.password===void 0){return}k=CryptoJS.SHA1(j.password).toString();j.password=k;return i.post("/user/login",j).then(function(l){var m;m=l.data;console.log(m);if(m.result==="success"){c.hasLogin=true;return c.currentUser={userName:m.userName,email:m.email,type:m.type}}else{if(m.result==="field_error"){return e.fieldInfo=m.field}else{return h.alert(m.error_msg)}}})};e.logout=function(){return i.post("/user/logout").then(function(j){var k;k=j.data;if(k.result==="success"){c.hasLogin=false;return c.currentUser={email:""}}})};return e.setupProfileEditor=function(){return g.setProfile(c.currentUser.userName)}}]);b.controller("UserProfileEditController",["$scope","$http","$element","$window","UserProfile",function(d,g,c,f,e){d.userEditDTO=0;d.$watch(function(){return e.getProfile()},function(){return d.userEditDTO=e.getProfile()},true);return d.edit=function(){var k,h,j,i;i=angular.copy(d.userEditDTO);if(i.newPassword===""){i.newPassword=void 0}if(i.newPasswordRepeat===""){i.newPasswordRepeat=void 0}if(i.newPassword===void 0&&i.newPasswordRepeat===void 0){i=_.omit(i,"newPassword");i=_.omit(i,"newPassowrdRepeat")}else{k=CryptoJS.SHA1(i.newPassword).toString();i.newPassword=k;h=CryptoJS.SHA1(i.newPasswordRepeat).toString();i.newPasswordRepeat=h}if(i.oldPassword===void 0){return}j=CryptoJS.SHA1(i.oldPassword).toString();i.oldPassword=j;return g.post("/user/edit",i).then(function(l){var m;m=l.data;if(m.result==="success"){return c.modal("hide")}else{if(m.result==="field_error"){return d.fieldInfo=m.field}else{return f.alert(m.error_msg)}}})}}]);b.factory("UserProfile",["$http","$window",function(e,d){var c;c=0;return{setProfile:function(f){return e.get("/user/profile/"+f).then(function(g){var h;h=g.data;if(h.result==="success"){return c=h.user}else{return d.alert(h.error_msg)}})},getProfile:function(){return c}}}]);b.directive("equals",function(){return{restrict:"A",require:"?ngModel",link:function(e,d,c,f){var g;if(f===void 0){return}e.$watch(c.ngModel,function(){return g()});c.$observe("equals",function(){return g()});return g=function(){var i,h;i=angular.copy(f.$viewValue);h=angular.copy(c.equals);if(h===void 0){h=""}if(i===void 0){i=""}return f.$setValidity("equals",i===h)}}}});b.directive("uiAvatar",function(){return{restrict:"A",scope:{email:"=",rating:"@",image:"@",size:"@"},controller:["$scope","$element",function(d,c){return d.$watch("email",function(){var e;if(d.size===void 0){d.size=c.width()}if(d.image===void 0){d.image="retro"}if(d.rating===void 0){d.rating="pg"}e="http://www.gravatar.com/avatar/"+(hex_md5(d.email))+".jpg?"+(d.size?"s="+d.size+"&":"")+(d.rating?"r="+d.rating+"&":"")+(d.image?"d="+encodeURIComponent(d.image):"");return c.attr("src",e)})}]}});b.directive("uiCodeHref",function(){return{restrict:"A",scope:{status:"="},controller:["$scope","$rootScope","$http","$modal",function(d,c,f,e){d.showHref=false;c.$watch("hasLogin",function(){if(c.hasLogin&&(c.currentUser.type===1||c.currentUser.userName===d.status.userName)){return d.showHref=true}else{return d.showHref=false}});return d.showCode=function(){var g;g=d.status.statusId;return e.open({templateUrl:"codeModal.html",controller:"CodeModalController",resolve:{statusId:function(){return g}}})}}],template:'<a href="#" ng-show="showHref" ng-click="showCode()">{{status.length}} B</a>\n<span ng-hide="showHref">{{status.length}} B</span>'}});b.controller("CodeModalController",["$scope","$http","$modalInstance","statusId",function(c,f,d,e){c.code="Loading...";return f.post("/status/info/"+e).then(function(g){var i,h,j;j=g.data;h="";if(j.result==="success"){h=j.compileInfo}else{h=j.error_msg}i="";if(j.result==="success"){i=j.code}else{i=j.error_msg}return c.code=i})}]);b.directive("uiCode",function(){return{restrict:"E",scope:{code:"="},controller:["$scope",function(d,c){d.prettifiedCode="";return d.$watch("code",function(){return d.prettifiedCode=prettyPrintOne(d.code.trim().escapeHTML())})}],template:'<pre ng-bind-html="prettifiedCode"></pre>',replace:true}});b.directive("uiContestAdminSpan",function(){return{restrict:"A",scope:{contestId:"=",isVisible:"="},controller:["$scope","$http",function(c,d){return c.editVisible=function(){var e;e="/contest/operator/"+c.contestId+"/isVisible/"+(!c.isVisible);return d.post(e).then((function(f){return function(g){if(g.data.result==="success"){return c.isVisible=!c.isVisible}}})(this))}}],template:'<div class="btn-toolbar" role="toolbar">\n  <div class="btn-group">\n    <button type="button" class="btn btn-default btn-sm" ng-click="editVisible()" style="padding: 1px 5px;">\n      <i class="fa" ng-class="{\n        \'fa-eye\': isVisible == true,\n        \'fa-eye-slash\': isVisible == false\n      }"></i>\n    </button>\n    <a href="/contest/editor/{{contestId}}" target="_blank"\n       class="btn btn-default btn-sm" style="padding: 1px 5px;"><i class="fa fa-pencil"></i></a>\n  </div>\n</div>'}});b.directive("uiDatetimepicker",function(){return{restrict:"A",link:function(d,c){return c.datetimepicker()}}});b.directive("uiDropdownMenu",function(){return{restrict:"A",link:function(d,c){return c.find("form").click((function(e){return function(f){return f.stopPropagation()}})(this))}}});b.directive("uiFlandre",function(){return{restrict:"A",scope:{content:"=ngModel",uploadUrl:"@"},controller:["$scope","$element",function(d,c){var f,e;d.mode="edit";d.previewContent="";f=c.find(".flandre-editor");d.togglePreview=function(){if(d.mode==="edit"){d.previewContent=d.content;return d.mode="preview"}else{return d.mode="edit"}};return e=new qq.FineUploaderBasic({button:c.find(".flandre-picture-uploader")[0],request:{endpoint:d.uploadUrl,inputName:"uploadFile"},validation:{allowedExtensions:["jpeg","jpg","gif","png"],sizeLimit:10*1000*1000},multiple:false,callbacks:{onComplete:function(l,k,j){var h,g,i;if(j.success==="true"){i="![title]("+j.uploadedFileUrl+")";g=f.getCursorPosition();h=f.val();h=h.insert(i,g);d.$apply(function(){return d.content=h});return f.setSelection(g+2,g+7)}else{return console.log(j)}}}})}],template:'<div class="panel panel-default">\n  <div class="panel-heading flandre-heading">\n    <div class="btn-toolbar" role="toolbar">\n      <div class="btn-group">\n        <button type="button" class="btn btn-default btn-sm"\n                ng-click="togglePreview()"\n                ng-class="{active: mode == \'preview\'}">Preview</button>\n      </div>\n      <div class="btn-group flandre-tools">\n        <span class="btn btn-default btn-sm"><i class="fa fa-smile-o"></i></span>\n        <span class="btn btn-default btn-sm flandre-picture-uploader"><i class="fa fa-picture-o"></i></span>\n      </div>\n    </div>\n  </div>\n  <textarea class="tex2jax_ignore form-control flandre-editor"\n            msd-elastic\n            ng-class="{\'flandre-show\': mode == \'edit\'}"\n            ng-model="content"></textarea>\n  <div class="flandre-preview" ng-class="{\'flandre-show\': mode == \'preview\'}">\n    <div ui-markdown content="previewContent"></div>\n  </div>\n</div>',replace:true}});b.directive("uiMarkdown",function(){return{restrict:"A",scope:{content:"="},link:function(d,c){var e;e=function(){c.prettify();return c.mathjax()};e();if(d.content!==void 0){return d.$watch("content",function(){var f;f=angular.copy(d.content);f=marked(f);c.empty().append(f);return e()},true)}}}});b.directive("uiPageInfo",function(){return{restrict:"A",scope:{pageInfo:"=",condition:"="},controller:["$scope",function(c){c.pageList=[];c.beginPages=1;c.endPages=1;c.$watch("pageInfo",function(){var d,e;if(c.pageInfo.totalPages>1){d=Math.max(1,c.pageInfo.currentPage-c.pageInfo.displayDistance);e=Math.min(c.pageInfo.totalPages,d+c.pageInfo.displayDistance*2);d=Math.max(1,e-c.pageInfo.displayDistance*2);c.beginPages=d;c.endPages=e;return c.pageList=_.range(d,e+1).map(function(f){return{page:f,active:f===c.pageInfo.currentPage}})}else{return c.pageList=""}},true);return c.jump=function(d){return c.condition.currentPage=d}}],template:'<ul class="pagination pagination-sm">\n  <li ng-class="{disabled: beginPages == 1}" ng-show="pageInfo.totalPages > 1"><a href="#" ng-click="jump(1)"><i class="fa fa-arrow-left"></i></a></li>\n  <li ng-repeat="page in pageList"\n      ng-class="{active: page.active}">\n    <a href="#" ng-click="jump(page.page)">{{page.page}}</a>\n  </li>\n  <li ng-class="{disabled: endPages == pageInfo.totalPages}" ng-show="pageInfo.totalPages > 1"><a href="#" ng-click="jump(pageInfo.totalPages)"><i class="fa fa-arrow-right"></i></a></li>\n</ul>'}});b.directive("uiProblemAdminSpan",function(){return{restrict:"A",scope:{problemId:"=",isVisible:"="},controller:["$scope","$http",function(c,d){return c.editVisible=function(){var e;e="/problem/operator/"+c.problemId+"/isVisible/"+(!c.isVisible);return d.post(e).then((function(f){return function(g){if(g.data.result==="success"){return c.isVisible=!c.isVisible}}})(this))}}],template:'<div class="btn-toolbar" role="toolbar">\n  <div class="btn-group">\n    <button type="button" class="btn btn-default btn-sm" ng-click="editVisible()" style="padding: 1px 5px;">\n      <i class="fa" ng-class="{\n        \'fa-eye\': isVisible == true,\n        \'fa-eye-slash\': isVisible == false\n      }"></i>\n    </button>\n    <a href="/problem/editor/{{problemId}}" target="_blank"\n       class="btn btn-default btn-sm" style="padding: 1px 5px;"><i class="fa fa-pencil"></i></a>\n  </div>\n</div>'}});b.directive("uiProblemDataUploader",function(){return{restrict:"E",scope:{uploadUrl:"@"},controller:["$scope","$element","$window",function(d,c,f){var e;d.hint="Upload";d.hasError=false;return e=new qq.FineUploaderBasic({button:c[0],request:{endpoint:d.uploadUrl,inputName:"uploadFile"},validation:{allowedExtensions:["zip"],sizeLimit:100*1000*1000},multiple:false,callbacks:{onComplete:function(i,h,g){if(g.success==="true"){return d.$apply(function(){d.hint="Total data: "+g.total;return d.hasError=false})}else{return d.$apply(function(){d.hint=g.error;return d.hasError=true})}},onProgress:function(j,i,g,h){return d.$apply(function(){var k;k=Math.round(g/h*100);return d.hint=""+k+" %"})},onError:function(i,h,g){return f.alert(g)}}})}],template:'<span class="btn"\n    ng-class="{\'btn-success\': hasError == false,\n\'btn-danger\': hasError == true}">\n  <i class="fa fa-upload"></i><span ng-bind="hint"></span>\n</span>',replace:true}});b.directive("uiReadmore",function(){return{restrict:"A",scope:{hasMore:"=",articleId:"="},link:function(d,c){if(d.hasMore){return c.empty().append('<a href="/article/show/'+d.articleId+'" target="_blank">Read more >></a>')}}}});b.directive("uiRejudgeButton",function(){return{restrict:"A",scope:{condition:"="},controller:["$scope","$http","$window",function(c,e,d){return c.rejudge=function(){return e.post("/status/count",c.condition).then(function(f){var g;g=f.data;if(g.result==="success"){if(confirm("Rejudge all "+g.count+" records")){return e.post("/status/rejudge",c.condition).then(function(h){g=h.data;if(g.result==="success"){return d.alert("Done!")}else{return d.alert(g.error_msg)}})}}else{return d.alert(g.error_msg)}})}}],template:'<button type="button" class="btn btn-danger btn-sm" ng-click="rejudge()">Rejudge</button>'}});b.directive("uiStatus",function(){return{restrict:"A",scope:{status:"="},controller:["$scope","$rootScope","$http","$modal",function(d,c,h,e){var g,f;d.showHref=false;g=function(){if(d.status.returnTypeId===7){if(c.hasLogin&&(c.currentUser.type===1||c.currentUser.userName===d.status.userName)){return d.showHref=true}else{return d.showHref=false}}else{return d.showHref=false}};c.$watch("hasLogin",function(){return g()});d.showCompileInfo=function(){var i;i=d.status.statusId;return e.open({templateUrl:"compileInfoModal.html",controller:"CompileInfoModalController",resolve:{statusId:function(){return i}}})};if([0,16,17,18].some(d.status.returnTypeId)){return f=setInterval(function(){var i;i={currentPage:null,startId:d.status.statusId,endId:d.status.statusId,userName:void 0,problemId:void 0,languageId:void 0,contestId:void 0,result:"OJ_ALL",orderFields:"statusId",orderAsc:"false"};return h.post("/status/search",i).then(function(j){var k;k=j.data;if(k.result==="success"&&k.list.length===1){d.status=k.list[0];g();if([0,16,17,18].none(d.status.returnTypeId)){return clearInterval(f)}}})},500)}}],template:'<a href="#" ng-show="showHref" ng-click="showCompileInfo()">{{status.returnType}}</a>\n<span ng-hide="showHref">{{status.returnType}}</span>'}});b.controller("CompileInfoModalController",["$scope","$http","$modalInstance","statusId",function(c,f,d,e){c.compileInfo="Loading...";return f.post("/status/info/"+e).then(function(g){var h,i;i=g.data;h="";if(i.result==="success"){h=i.compileInfo}else{h=i.error_msg}return c.compileInfo=h})}]);b.directive("uiTime",function(){return{restrict:"A",scope:{time:"=",inline:"=",format:"@",show:"@",change:"@"},controller:["$scope",function(c){c.timeString="";c.showRelativeTime=function(){return c.timeString=Date.create(c.time).relative()};c.showRealTime=function(){return c.timeString=Date.create(c.time).format("{yyyy}-{MM}-{dd} {HH}:{mm}:{ss}")};c.showTime=function(){if(c.show==="real"){return c.showRealTime()}else{return c.showRelativeTime()}};c.$watch("time",function(){return c.showTime()});c.mouseOver=function(){if(c.change==="false"){return}if(c.show==="real"){return c.showRelativeTime()}else{return c.showRealTime()}};c.mouseLeave=function(){if(c.change==="false"){return}return setTimeout(function(){return c.$apply(function(){return c.showTime()})},400)};return c.getInline=function(){if(c.inline){return"inline"}else{return"block"}}}],template:'<div ng-mouseover="mouseOver()"\n     ng-mouseleave="mouseLeave()"\n     style="width: 100%;"\n     ng-style="{display: getInline()}">{{timeString}}</div>'}});b.directive("uiTimeLength",function(){return{restrict:"A",scope:{length:"="},controller:["$scope",function(c){c.timeString="";c.$watch("length",function(){return c.showTimeLength()});return c.showTimeLength=function(){var h,d,f,g,e;f=parseInt(c.length);e=f%60;f=(f-e)/60;g=f%60;f=(f-g)/60;d=f%24;f=(f-d)/24;h=f;c.timeString="";if(h>0){c.timeString=c.timeString+h+(" "+(h===1?"day":"days")+" ")}return c.timeString=c.timeString+"%d:%02d:%02d".sprintf(d,g,e)}}],template:"{{timeString}}"}});b.directive("uiPenalty",function(){return{restrict:"A",scope:{penalty:"="},controller:["$scope",function(c){c.timeString="";c.$watch("penalty",function(){return c.showPenaltyMinutes()});c.showPenaltyMinutes=function(){return c.timeString=Math.round(c.penalty/60)};c.showPenalty=function(){var d,f,g,e;f=parseInt(c.penalty);e=f%60;f=(f-e)/60;g=f%60;f=(f-g)/60;d=f;c.timeString="";return c.timeString=c.timeString+"%d:%02d:%02d".sprintf(d,g,e)};c.mouseOver=function(){return c.showPenalty()};return c.mouseLeave=function(){return setTimeout(function(){return c.$apply(function(){return c.showPenaltyMinutes()})},400)}}],template:'<span ng-mouseover="mouseOver()" ng-mouseleave="mouseLeave()">{{timeString}}</span>'}});b.directive("uiUserAdminSpan",function(){return{restrict:"E",scope:{user:"="},controller:["$scope","$http","$modal","$window",function(c,f,d,e){return c.showEditor=function(){var g;g=angular.copy(c.user.userName);return f.get("/user/profile/"+g).then(function(h){var i;i=h.data;if(i.result==="success"){return d.open({templateUrl:"userAdminModalContent.html",controller:["$scope","$http","$modalInstance",function(j,l,k){j.userEditDTO=i.user;j.fieldInfo=[];j.edit=function(){var o,m,n;n=angular.copy(j.userEditDTO);if(n.newPassword===""){n.newPassword=void 0}if(n.newPasswordRepeat===""){n.newPasswordRepeat=void 0}if(n.newPassword===void 0&&n.newPasswordRepeat===void 0){n=_.omit(n,"newPassword");n=_.omit(n,"newPassowrdRepeat")}else{o=CryptoJS.SHA1(n.newPassword).toString();n.newPassword=o;m=CryptoJS.SHA1(n.newPasswordRepeat).toString();n.newPasswordRepeat=m}console.log(n);return l.post("/user/adminEdit",n).then(function(p){i=p.data;if(i.result==="success"){return k.close()}else{if(i.result==="field_error"){return j.fieldInfo=i.field}else{return e.alert(i.error_msg)}}})};return j.close=function(){return k.dismiss("close")}}]})}else{return e.alert(i.error_msg)}})}}],template:'<div class="btn-toolbar" role="toolbar" style="position: absolute; top: 12px; right: 30px;"\n     ng-show="$root.isAdmin">\n  <div class="btn-group">\n    <button type="button" class="btn btn-default btn-sm" ng-click="showEditor()">\n      <i class="fa fa-pencil"></i>\n    </button>\n  </div>\n</div>'}});b.directive("uiValidateInfo",function(){return{restrict:"E",scope:{value:"=","for":"@"},link:function(c){c.isInvalid=false;c.message="";return c.$watch("value",function(){var d;d=_.findWhere(c.value,{field:c["for"]});if(d!==void 0){c.message=d.defaultMessage;return c.isInvalid=true}})},template:'<span class="help-block" ng-show="isInvalid" ng-bind="message"></span>'}});b.directive("uiYesNoRadio",function(){return{restrict:"E",scope:{ngModel:"="},controller:["$scope",function(c){c.options=[{key:"All",value:void 0},{key:"Yes",value:true},{key:"No",value:false}];return c.select=function(d){return c.ngModel=d}}],replace:true,template:'<div class="btn-group pull-right" data-toggle="buttons">\n  <button type="button" class="btn  btn-info btn-sm" ng-class="{active: option.value == ngModel}"\n          ng-repeat="option in options"\n          ng-click="select(option.value)"\n          ng-bind="option.key">\n  </button>\n</div>'}})}).call(this);