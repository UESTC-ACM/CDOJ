...
function getContestId(contestId) {
    var html = $('<td></td>');
    if (contestId != null)
        html.append('<a href="/contest/show/'+contestId+'">' + contestId);
    return html;
}
...
function getHTML(value) {
    ...
    html.append(getContestId(value.contestId));
...
function refreshStatusList(condition) {
    ...
    $.post('/admin/status/search', condition, function (data) {
...
function rejudge(condition) {
    $.post('/admin/status/countRejudge', condition, function (data) {
        if (data.result == "error") {
            alert(data.error_msg);
            return;
        }

        var result = confirm('Are you sure to rejudge this ' + data.count + ' status?');
        if (result == true) {
            $.post('/admin/status/rejudge', condition, function (data) {
                if (data.result == "error") {
                    alert(data.error_msg);
                    return;
                }
                alert('Rejudge success!');
            });
            alert('We will rejudge this status right now, PLEASE DO NOT DO ANY OPERATOR UNTIL REJUDGE SUCCESSFUL!');
        }
    });
}
...
$(document).ready(function () {
    ...
    $('input#rejudge').setButton({
        callback: function () {
            currentCondition = $('#statusCondition').getFormData();
            if (currentCondition["statusCondition.userName"] == '')
                currentCondition["statusCondition.userName"] = undefined;
            rejudge(currentCondition);
        }
    });
