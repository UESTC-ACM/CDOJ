...
public class TrainingContestUtilsTest implements TrainingStatusConditionAware,
		TrainingContestConditionAware, TrainingUserConditionAware,
		...
		for (int i = 1; i <= 10; i++) {
			for (int j = 1; j <= 10; j++) {
				...
				trainingStatus.setTrainingUserByTrainingUserId(trainingUserDAO
						.get(j));
				trainingStatus
						.setTrainingContestByTrainingContestId(trainingContestDAO
								.get(i));
	...
	public void testDeleteByCondition() throws AppException {
		...
		trainingStatusDAO.deleteEntitiesByCondition(trainingStatusCondition
				.getCondition());
	}
